...
(() => {
  ...
  const exact = new Map([
    ...
    ["インカメで撮影", "Record with front camera"], ["音声なし・通し終了まで", "No audio · until the run ends"],
    ["音声なし・音源停止まで", "No audio · until the music stops"],
    ...
    ["選んだ画角で上の確認映像・撮影中プレビュー・保存映像を表示します。録画は楽曲の再生開始に合わせて始まり、停止すると終了します。", "The selected framing is used for the setup preview, live preview, and saved video. Recording starts with music playback and ends when the music stops."],
    ...
    ["通し映像", "Full-run videos"], ["映像データが見つかりません", "Video data not found"],
  ...
  const rules = [
    ...
    [/^全体 (\d+)\/(\d+)本$/, "$1/$2 total"],
    [/^音声なし \/ (.+) \/ (.+)$/, "No audio / $1 / $2"],
    [/^インカメ \/ 音声なし \/ (.+)$/, "Front camera / no audio / $1"],
    [/^(.+) \/ インカメ・音声なし$/, "$1 / front camera · no audio"],
    [/^● 映像 (.+)$/, "● Video $1"],
