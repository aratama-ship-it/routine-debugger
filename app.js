...
const APP_VERSION = "v116"; // 要望フォーム等で自動送信するアプリ版
...
if ("preservesPitch" in musicPlayer) musicPlayer.preservesPitch = true;
...
function go(name, params = {}) {
  ...
  if (view.name === "part" && name !== "part") musicPlayer.playbackRate = 1;
...
function renderEdit() {
  ...
  return `
    ...
    <div class="card">
      ...
      <input type="text" value="${esc(draft.name)}" placeholder="例: 2026ステージ用 4分" oninput="draft.name=this.value">
    </div>
    ${musicCard}
    <div class="card">
...
const PART_PLAYBACK_RATES = [0.5, 0.75, 1, 1.25];

function partPlaybackRate(rt) {
  const raw = Number(rt && rt.partPlaybackRate);
  return PART_PLAYBACK_RATES.includes(raw) ? raw : 1;
}
...
window.partSetPlaybackRate = (rate) => {
  ...
  if (!rt || !PART_PLAYBACK_RATES.includes(next)) return;
  rt.partPlaybackRate = next;
  musicPlayer.playbackRate = next;
  saveState(); render();
  ...
  return `
    ...
    <div class="card music-card">
      ...
      <div class="part-speed-row">
        <span>再生速度</span>
        <div role="group" aria-label="パート練習の再生速度">
          ${PART_PLAYBACK_RATES.map((rate) => `<button class="${rate === playbackRate ? "selected" : ""}"
