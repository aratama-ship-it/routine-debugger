...
const APP_VERSION = "v35"; // 要望フォーム等で自動送信するアプリ版
...
window.loadSampleTricks = async () => {
  if (!location.protocol.startsWith("http")) return alert("ファイルから直接開いているため、サンプルを取得できません。\n\n公開版URLで開いてください:\nhttps://aratama-ship-it.github.io/routine-debugger/");
...
window.loadSampleSet = async () => {
  ...
  let music = null;
  try {
    const resp = await fetch("samples/challie-lav.mp3"); // サンプル楽曲=開発者本人の楽曲(リリース時に追加予定あり)
    if (resp.ok) {
      const blob = await resp.blob();
      const mid = uid();
      if (await blobPut(mid, blob)) music = { blobId: mid, name: "challie lav.mp3" };
    }
  } catch (_) {}
