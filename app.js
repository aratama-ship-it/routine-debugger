...
const APP_VERSION = "v57"; // 要望フォーム等で自動送信するアプリ版
...
function editorTickUI() {
  ...
  if (ph) {
    ...
    const base = isFinite(musicPlayer.duration) && musicPlayer.duration ? musicPlayer.duration : Math.max(total, 1);
...
function musicRafTick() {
  ...
  if (ph && draft) {
    const total = draft.steps.reduce((a, s) => a + stepDur(s), 0);
    const base = isFinite(musicPlayer.duration) && musicPlayer.duration ? musicPlayer.duration : Math.max(total, 1);
    ph.style.left = Math.min(100, (musicPlayer.currentTime / base) * 100) + "%";
  }
...
function renderEdit() {
  ...
  if (hasEditorMusic && draft.steps.length) {
    ...
    const songDur = (musicLoadedFor && String(musicLoadedFor).startsWith("edit") && isFinite(musicPlayer.duration)) ? musicPlayer.duration : null;
  ...
  const musicCard = hasEditorMusic ? `
    <div class="card music-card">
      ...
      <div class="music-foot">
        <span class="mf-name" title="${esc(trackName)}">♪ ${esc(trackName)}</span>
  ...
  return `
    ...
    <div class="card">
      <h2>デザイン(このルーティンの世界観)${infoBtn("theme")}</h2>
...
window.playSample = async (i) => {
  if (audioPlayingId === "sample:" + i) { libAudio.pause(); audioPlayingId = null; return render(); }
...
function renderHelp() {
  return `
    ...
    <div class="card">
      <h2>楽曲と録音</h2>
      <div class="help-body">編集画面で音源(MP3等)を添付すると、通し練習中に再生できます。<b>失敗をタップした瞬間の曲位置(♪1:23)が自動で記録</b>され、曲は自動停止。クリーン/中止を記録すると曲は頭に戻るので、次の通しは▶を押すだけです。<br><br>音源は「音源ライブラリ」に貯めて、どのルーティンでも「♪ ライブラリから」で使い回せます。マイク録音した音源もライブラリに追加できます。<br><br>音源はこの端末のブラウザ内にのみ保存され、JSONバックアップには含まれません。</div>
    </div>
    <div class="card">
      <h2>パート練習</h2>
