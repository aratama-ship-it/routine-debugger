...
const APP_VERSION = "v89"; // 要望フォーム等で自動送信するアプリ版
...
function migrateState() {
  ...
  if (state.builder && state.builder.music) normalizeMusicMeta(state.builder.music);
  return sampleDataChanged;
}
...
let musicTrimMeta = null;    // 現在ロード中の曲のトリム情報(routine.music / draft.music / builder.music)
...
function editorTickUI() {
  ...
  // タイムラインのプレイヘッド(統合したビルダー機能)
  const ph = document.getElementById("edit-tl-playhead");
  if (ph) {
    const total = draft.steps.reduce((a, s) => a + stepDur(s), 0);
    const base = musicEffectiveDuration() || Math.max(total, 1);
    ph.style.left = Math.min(100, (cur / base) * 100) + "%";
  }
}
...
function practiceStepName(rt, step) {
  if (!isSlot(step)) return step.name || "名称未設定";
  // A/Bを使わない間は、どの画面の現在技表示でも選択肢Aを通常の技として扱う。
  // 保存済みの選択肢とセッション既定値は消さず、ONに戻したときに復元する。
  if (!state.settings.showSlots) return (step.options[0] && step.options[0].name) || step.name || "名称未設定";
  if (view.name === "edit") return stepLabel(step);
  if (view.name === "record") {
    const sess = activeSession(rt.id);
    const optId = currentChoice(sess, step);
    const opt = step.options.find((o) => o.id === optId) || step.options[0];
    return (opt && opt.name) || stepLabel(step);
  }
...
function practiceNowDockHtml() {
  return `<section class="practice-now paused" id="practice-now" aria-live="polite" aria-atomic="true">
...
async function updatePracticeNowUI() {
  ...
  const media = document.getElementById("practice-now-media");
  const trick = current.step.trickId ? (state.tricks || []).find((t) => t.id === current.step.trickId) : null;
  if (!media) return;
  ...
  if (practiceVideoFailed.has(trick.id)) {
    media.innerHTML = `<span class="practice-video-loading">${uiText("動画を読み込めません")}</span>`;
    return;
  }
  media.innerHTML = `<span class="practice-video-loading">${uiText("動画を準備中…")}</span>`;
...
function musicRafTick() {
  ...
  if (ph && draft) {
    const total = draft.steps.reduce((a, s) => a + stepDur(s), 0);
    const base = effectiveDur || Math.max(total, 1);
    ph.style.left = Math.min(100, (rel / base) * 100) + "%";
  }
...
window.musicToggle = () => {
  if (musicPlayer.paused) {
    ...
    ensureAudioGraph(); musicPlayer.play();
  }
...
const INFO = {
  timeline: { t: "タイムライン", b: "各技の長さ(動画リンクは自動、それ以外は既定値)を足し合わせて曲位置を計算し、バーで可視化します。ステップの「♪ 技の長さから曲位置を自動セット」で全ステップの♪キューに反映されます。プレイヤーで再生すると、バー上に現在位置(緑の線)が表示されます。" },
  steps: { t: "ステップの並べ替え", b: "番号の下の <span style=\"color:var(--muted)\">⠿</span> を上下にドラッグすると、技の順番を入れ替えられます。" },
  meter: { t: "失敗率メーターの見方", b: "各バーは失敗率(左=0%、右=100%)です。<br><br><b>オレンジの帯</b> = まだ不確かな範囲(95%区間)。帯が広いほど本数が少なく、まだ断定できません。<br><b>縦線</b> = 実測の失敗率。<br><br>分母は「そのステップに到達した通し数」です(途中で中止した通しは、その先の分母に入りません)。" },
  ...
  editorFeatures: { t: "ルーティンで使う機能", b: "人によっては使わない機能を、初期状態では隠しています。<br><br><b>リスク度</b> = 各技に危険度(1〜5)を付けて、分析で失敗率とのズレを見る機能。<br><b>A/B分岐</b> = 本番でどちらの技をやるか選べるステップを作る機能。<br><br>必要な人だけONにしてください。設定画面のほか、ルーティンの編集・通し練習・パート練習・分析にある右上メニューからも切り替えられます。OFFにしても、すでに設定済みのリスク度やA/B分岐のデータは消えません。" },
  ...
  reset: { t: "初期化", b: "まっさらな状態から試し直したいとき・サンプル一式を入れ直したいときに。ルーティン・記録・技の動画・録音・楽曲・設定がすべて消えます(元に戻せません)。" },
};
const INFO_EN = {
  timeline: { t: "Timeline", b: "The timeline adds each skill duration to estimate its position in the music. Use “Set cues from skill durations” to write those positions to every step. The playhead shows the current position while music is playing." },
  steps: { t: "Reordering steps", b: "Drag the ⠿ handle below the step number to change the order." },
  meter: { t: "Reading the issue-rate meter", b: "The vertical line is the observed issue rate. The orange range is its 95% uncertainty interval. A wider range means there are not enough runs to draw a firm conclusion. The denominator is the number of runs that reached that step." },
  ...
  editorFeatures: { t: "Routine features", b: "Risk rating compares your expectation with the observed issue rate. A/B branch lets you choose between two skills for a run. You can toggle these features from Settings or the menu on Edit, Full Run, Section Practice, and Analysis. Turning them off does not erase saved values." },
...
// ルーティン画面共通の右上メニュー。任意機能を練習・編集・分析のどこからでも切り替える。
function routineMenuAction(routineId, before = "") {
  return `<div class="routine-head-actions">${before}<button class="routine-menu-btn"
    onclick="showRoutineMenu('${routineId || ""}')" aria-label="ルーティンの機能設定を開く" title="機能設定">
...
window.showRoutineMenu = (routineId) => {
  showSheet(`
    <h3>ルーティンの機能設定</h3>
    <div class="sheet-sub">このルーティンで表示する機能を設定します</div>
    <section class="routine-menu-section" aria-labelledby="routine-feature-title">
      ...
      <div class="routine-menu-toggle-list">
        ${switchRow("リスク度", "事前予想と実際の失敗率を比べる", "showRisk")}
        ${switchRow("A/B分岐", "本番で使う技を選択肢から切り替える", "showSlots")}
      </div>
...
function renderHome() {
  ...
  const previousButton = previousRoutine ? `
    <button class="home-recent-button" onclick="go('record',{id:'${previousRoutine.id}'})">
      <span class="home-paper-grip" aria-hidden="true"><i></i><i></i><i></i></span>
      <span class="home-recent-copy">
        <b data-user-text>${esc(previousRoutine.name)}</b>
        <small>${liveSession ? "練習中・続きを開く" : "もう一度練習する"}</small>
      </span>
      ...
      <div class="home-paper-sheet">
        ...
        <main class="home-simple-main">
          <button class="home-practice-button" onclick="go('routines')">
            <span class="home-practice-copy"><small>START</small><span>ルーティン練習をする</span></span>
            <span class="home-button-arrow" aria-hidden="true">→</span>
          </button>
...
window.deleteRoutine = async (id) => {
  ...
  if (!appConfirm(msg)) { closeAllSwipes(); return; }
...
function draftStarts() { let t = 0; return draft.steps.map((s) => { const st = t; t += stepDur(s); return st; }); }
...
window.editorAutoCue = () => {
  ...
  if (!appConfirm("各技の長さから曲位置(♪)を自動計算して、全ステップのキューを上書きします。よいですか?")) return;
  const starts = draftStarts();
  draft.steps.forEach((s, i) => { s.cue = Math.round(starts[i] * 10) / 10; });
  render();
  toast("曲位置を自動セットしました");
};
...
function renderEdit() {
  ...
  if (!draft || draft._for !== (view.params.id || "new")) {
    draft = rt
      ? { _for: rt.id, id: rt.id, name: rt.name, steps: latestVersion(rt).steps.map((s) => ({ ...s })),
          music: rt.music ? { ...rt.music } : null, countdownSeconds: routineCountdownSeconds(rt), _newMusicFile: null }
      : { _for: "new", name: "", steps: [], music: null,
          countdownSeconds: DEFAULT_RUN_COUNTDOWN, _newMusicFile: null };
  }
  ...
  const showRisk = !!state.settings.showRisk;
  const showSlots = !!state.settings.showSlots;
  const stepRows = draft.steps.map((s, i) => {
    ...
    const nameVal = collapsedSlot ? (s.options[0].name || "") : s.name;
    const namePh = collapsedSlot ? "技名" : (isSlot(s) ? "分岐の名前(例: ラスト技)" : s.kind === "transition" ? "移行(例: 持ち替え)" : "技名");
    const nameOnchange = collapsedSlot ? `draft.steps[${i}].options[0].name=this.value` : `draft.steps[${i}].name=this.value`;
    return `
    <div class="editor-step">
      ...
      <div class="es-row2">
        ...
        ${collapsedSlot ? "" : `<button class="kind-toggle ${s.kind === "trick" ? "t" : ""}" onclick="toggleKind(${i})">${s.kind === "trick" ? "技" : "移行"}</button>`}
        ${showSlots ? (isSlot(s)
        ...
        ${s.trickId && (state.tricks || []).some((t) => t.id === s.trickId)
          ? `<button class="mini-btn play" aria-label="この技を上部でプレビュー" onclick="editorPreviewTrick(${i})">▶</button>
  ...
  // タイムラインの横バー(再生ボタンと縦に並べる)。技リストは廃し、位置の可視化バーのみ
  let tlBar = "";
  if (hasEditorMusic && draft.steps.length) {
    ...
    const segs = draft.steps.map((s, i) => {
      const left = (starts[i] / base) * 100, w = Math.max(0.5, (stepDur(s) / base) * 100);
      return `<div class="tl-seg ${s.trickId ? "" : "gap"}" style="left:${left}%;width:${Math.min(w, 100 - left)}%"></div>`;
    }).join("");
    tlBar = `
      <div class="tl-caption">合計 ${fmtTime(total)}${songDur ? ` / 曲 ${fmtTime(songDur)}` : ""}${over ? ` <span style="color:var(--danger);margin-left:6px">曲より長い</span>` : ""}${infoBtn("timeline")}</div>
      <div class="tl-bar">${segs}<div id="edit-tl-playhead"></div></div>`;
  }
  ...
  const musicCard = hasEditorMusic ? `
    <div class="card music-card">
      <div class="music-row">
        ...
        <div class="music-time" style="font-size:19px"><span id="music-cur">${fmtTimeFine(musicCurrentTime())}</span><span class="dur"> / <span id="music-dur">${fmtTime(musicEffectiveDuration())}</span></span></div>
      ...
      <div class="volume-row">
        ...
        <input type="range" id="music-vol" min="0" max="1" step="0.02" value="${musicVolume}"
          aria-label="楽曲の音量" oninput="musicSetVolume(this.value)">
        <span class="vol-ico">🔊</span>
      </div>
      ${tlBar}
  ...
  return `
    ...
    <div class="card">
      ...
      ${hasEditorMusic && draft.steps.length ? `<button class="btn small primary" style="margin-top:10px;width:100%" onclick="editorAutoCue()">♪ 技の長さから曲位置を自動セット</button>` : ""}
    </div>
...
window.confirmRunStart = (routineId) => {
  ...
  showSheet(`
    <h3>通し練習を始めますか？</h3>
    <div class="sheet-sub">${esc(rt.name)}</div>
...
function currentChoice(sess, st) {
  if (!state.settings.showSlots) return st.options[0].id;
...
function renderRecord() {
  ...
  const stepBtns = ver.steps.map((s, i) => {
    ...
    if (isSlot(s) && state.settings.showSlots) {
      ...
      return `<div class="step-btn slot" onclick="tapStep(${i})">
        ...
        ${hit ? `<span class="badge hit">記録済</span>` : showRisk && risk >= 3 ? `<span class="badge risk-${risk}">${RISK_LABEL[risk]}</span>` : ""}
      </div>`;
    }
    if (isSlot(s)) {
      ...
      return `<div class="step-btn ${s.kind}" onclick="tapStep(${i})">
        <span class="no">${i + 1}</span><span class="nm">${s.cue != null ? `<span class="cue-chip">♪${fmtCue(s.cue)}</span> ` : ""}${esc(selOpt.name || s.name)}</span>
        ${hit ? `<span class="badge hit">記録済</span>` : showRisk && risk >= 3 ? `<span class="badge risk-${risk}">${RISK_LABEL[risk]}</span>` : ""}
      </div>`;
    }
    ...
    return `<div class="step-btn ${s.kind}" onclick="tapStep(${i})">
      ...
      ${hasVideo ? `<button class="mini-btn play" aria-label="${esc(s.name)}の動画を再生" onclick="event.stopPropagation();playTrickVideo('${s.trickId}')">▶</button>` : ""}
      ${hit ? `<span class="badge hit">記録済</span>` : showRisk && risk >= 3 ? `<span class="badge risk-${risk}">${RISK_LABEL[risk]}</span>` : ""}
    </div>`;
  }).join("");
  ...
  return `
    <div class="topbar"><button class="back-btn" onclick="endSessionAsk('${rt.id}')">戻る</button>
      <h1 class="record-mode-head"><span>通し練習モード</span><small>${esc(rt.name)}</small></h1>
    ...
    <div class="card">
      <h2>${runActive ? "失敗した場所をタップ" : "スタート後、失敗した場所をタップ"}</h2>
...
window.tapStep = (stepIndex) => {
  ...
  const slotChips = isSlot(step) && state.settings.showSlots ? `
...
function renderStats() {
  ...
  if (st.total === 0) {
    return `
      <div class="topbar"><button class="back-btn" onclick="go('routines')">戻る</button>
        <h1>${esc(rt.name)} 分析</h1>${routineMenuAction(rt.id)}</div>
      ${verSelect}
  ...
  const stepRows = st.steps.map((s) => {
    ...
    if (s.options && showSlots) {
      ...
      const optRows = s.options.map((o) => {
        ...
        const oGap = showRisk && o.opt.risk != null ? gapNote(o.opt.risk, o.failed, o.reached, oRate) : "";
        ...
        return `<div class="slot-opt-stat">
          <div class="head"><span class="nm">└ ${esc(o.opt.name)}</span>
            ${showRisk && o.opt.risk != null ? `<span class="risk-chip risk-${o.opt.risk}">${RISK_LABEL[o.opt.risk]}</span>` : ""}
    ...
    if (s.options) {
      ...
      const collapsedName = s.step.name || "選択ステップ";
      return `<div class="step-stat ${s.step.kind}" ${openDetail}>
    ...
    const gap = showRisk && risk != null ? gapNote(risk, s.failed, s.reached, rate) : "";
    return `<div class="step-stat ${s.step.kind}" ${openDetail}>
      <div class="head"><span class="nm">${s.index + 1}. ${esc(s.step.name)}</span>
        ${showRisk && risk != null ? `<span class="risk-chip risk-${risk}">${RISK_LABEL[risk]}</span>` : ""}
        <span class="kn">${knTxt(s)} ›</span></div>
      ${s.reached ? bar : ""}
      ${timeChips}
  ...
  return `
    <div class="topbar"><button class="back-btn" onclick="go('routines')">戻る</button>
      <h1>${esc(rt.name)} 分析</h1>${routineMenuAction(rt.id, `<span class="sub">v${verIndex}</span>`)}</div>
    ${verSelect}
    ${overview}
    <div class="card">
      <h2>ステップ別の失敗${infoBtn("meter")}</h2>
      ${stepRows}
    </div>
    ${recCard}
...
function renderPart() {
  ...
  if (!rt.music) {
    return `
      <div class="topbar"><button class="back-btn" onclick="go('routines')">戻る</button>
        <h1>${esc(rt.name)} パート練習</h1>${routineMenuAction(rt.id)}</div>
...
function renderStepDetail() {
  ...
  const showSlots = !!state.settings.showSlots;
  const detailStepName = isSlot(step) && !showSlots ? (step.name || "選択ステップ") : stepLabel(step);
...
function renderHistory() {
  ...
  const blocks = sessions.map((sess) => {
    ...
    const runRows = sess.runs.map((run, ri) => {
      ...
      const slotFix = !state.settings.showSlots ? "" : ver.steps.filter(isSlot).map((st) => {
    ...
    return `<div class="card">
      ...
      ${sess.note ? `<div class="note-line">${isEnglish() ? "Conditions" : "条件"}: ${esc(sess.note)}</div>` : ""}
      ${sess.review ? `<div class="note-line">${isEnglish() ? "Review" : "振り返り"}: ${esc(sess.review)}</div>` : ""}
      ${sess.nextPlan ? `<div class="note-line plan">${isEnglish() ? "Try next" : "次回試すこと"}: ${esc(sess.nextPlan)}</div>` : ""}
  ...
  return `
    <div class="topbar"><button class="back-btn" onclick="go('stats',{id:'${rt.id}'})">戻る</button>
      <h1>${esc(rt.name)} 履歴</h1>${routineMenuAction(rt.id)}</div>
    ${blocks}
    `;
}
...
function showLoading(msg) {
  ...
  el.innerHTML = `<div class="spin"></div><div class="msg">${esc(uiText(msg || "読み込み中…"))}</div>`;
...
window.sheetTrimAudio = async (id) => {
  const a = (state.audios || []).find((x) => x.id === id);
  if (!a) return;
  await openMusicTrim(a, await blobGet(a.blobId), "library", id);
};
window.sheetTrimRoutineMusic = async () => {
  ...
  await openMusicTrim(draft.music, blob, "draft", draft._for);
};
window.sheetTrimBuilderMusic = async () => {
  const b = builderState(); if (!b.music) return;
  await openMusicTrim(b.music, await blobGet(b.music.blobId), "builder", "builder");
};
...
window.pickLibraryMusic = async (id, target) => {
  ...
  await attachMusicFile(file, target, a);
};
...
function sampleDirectChoiceStep(slot) {
  ...
  return {
    ...
    risk: opt?.risk, trickId: trick?.id,
  };
}
...
function seedSampleHistory(rt, force = false) {
  ...
  plans.forEach((plan, sessionIndex) => {
    ...
    state.sessions.push({
      ...
      date: new Date(startedAt).toISOString().slice(0, 10), startedAt,
  ...
  const rows = tricks.map((t) => `
    <div class="trick-row">
      <div class="head">
        ...
        <button class="btn small ${trickPlayingId === t.id ? "" : "video-play-btn"}" aria-label="${esc(t.name)}の動画を${trickPlayingId === t.id ? "閉じる" : "再生"}" onclick="trickPlay('${t.id}')">${trickPlayingId === t.id ? "閉じる" : "▶"}</button>
...
async function loadBuilderMusic() {
  ...
  if (musicObjectUrl) URL.revokeObjectURL(musicObjectUrl);
  musicObjectUrl = URL.createObjectURL(blob);
  musicTrimMeta = normalizeMusicMeta(b.music);
  musicPlayer.src = musicObjectUrl;
...
window.builderAttachMusic = async (input) => {
  ...
  if (!(await blobPut(blobId, file))) return toast("音源を保存できませんでした");
  if (b.music) blobDel(b.music.blobId);
  b.music = { blobId, name: file.name, fullDuration: null, trimStart: 0, trimEnd: null, duration: null };
  musicLoadedFor = null;
  saveState();
  loadBuilderMusic();
};
window.builderPickTrick = () => {
  ...
  showSheet(`
    <h3>技を配置</h3>
    <div class="sheet-sub">タップで末尾に追加 / 再生マークで動画を確認</div>
    ${tricks.map((t) => `
      <div class="pick-trick-row" onclick="builderAddTrick('${t.id}')">
        <span class="nm">${esc(t.name)}</span>
        <span class="kn">${t.duration.toFixed(1)}s</span>
        <button class="mini-btn play" aria-label="${esc(t.name)}の動画を再生" onclick="event.stopPropagation();playTrickVideo('${t.id}')">▶</button>
      </div>`).join("")}
    <div style="height:10px"></div>
    <button class="btn ghost" onclick="hideSheet()">閉じる</button>`);
};
...
window.builderSeekItem = (i) => {
  ...
  musicSetTime(builderStarts(b)[i]);
  ensureAudioGraph();
  musicPlayer.play();
};
window.builderClearAsk = () => {
  if (!appConfirm("タイムラインを空にしますか?(音源は残ります)")) return;
  builderState().items = [];
  saveState(); render();
};
...
function builderTickUI() {
  ...
  const effectiveDur = musicEffectiveDuration();
  if (ph && effectiveDur > 0) {
    ph.style.left = `${Math.min(100, (cur / effectiveDur) * 100)}%`;
  }
...
window.builderExport = async () => {
  ...
  if (b.music) {
    const blob = await blobGet(b.music.blobId);
    if (blob) { const bid = uid(); if (await blobPut(bid, blob)) music = { ...b.music, blobId: bid }; }
  }
  const starts = builderStarts(b);
...
function renderBuilder() {
  ...
  const songDur = musicLoadedFor === "builder" ? musicEffectiveDuration() : null;
  const starts = builderStarts(b);
  ...
  const rows = b.items.map((it, i) => `
    <div class="bi-row" onclick="builderSeekItem(${i})">
      ...
      ${it.trickId
        ? `<button class="mini-btn play" aria-label="${esc(it.name)}の動画を再生" onclick="event.stopPropagation();playTrickVideo('${it.trickId}')">▶</button>`
  ...
  return `
    ...
    <div class="card music-card">
      ${b.music ? (musicMissing && musicLoadedFor === "builder"
        ...
        : `<div class="music-name">♪ ${esc(b.music.name)}${musicMetaIsTrimmed(b.music) ? " ✂" : ""}</div>
           <div class="music-time big"><span id="music-cur">${fmtTimeFine(musicCurrentTime())}</span><span class="dur"> / <span id="music-dur">${fmtTime(musicEffectiveDuration())}</span></span></div>
           ...
           <div class="music-controls">
             ...
             <button class="music-pill" onclick="musicStop()">■ 停止</button>
             <button class="music-pill" onclick="sheetTrimBuilderMusic()">編集</button>
           </div>`)
      : `<div class="row-2">
...
function renderHelpEnglish() {
  return `
    ...
    <div class="card"><h2>Full-run workflow</h2>
      ...
      2. Tap the large vermilion start button, confirm the countdown, and begin. You can change the countdown with − / + on the confirmation screen.<br>
      3. For a clean run, tap <b>Clean</b> below the skill list.<br>
      4. If something happens, tap the relevant skill, choose the issue type, and add optional cause tags or a note.<br>
      5. Start the next run, or end the session and leave a review and one thing to try next time.</div></div>
    <div class="card"><h2>Music and recordings</h2>
      <div class="help-body">Attach audio in Routine Edit. Volume is shared across Edit, Full Run, and Section Practice. When you log an issue, the current music position is saved automatically. Audio Library lets you reuse music and microphone recordings across routines. Audio stays on this device and is not included in JSON backups.</div></div>
    <div class="card"><h2>Section Practice</h2>
      <div class="help-body">Set points A and B on the music and loop that range. Drag either handle for quick adjustment. Section Practice is deliberately excluded from analysis because its conditions differ from a full run.</div></div>
    ...
    <div class="card"><h2>Saving your data</h2>
      <div class="help-body">Data is stored in this browser. Export a JSON backup regularly and before changing devices. Audio is not included, so reattach music after restoring.</div></div>`;
}
  ...
  return `
    ...
    <div class="card">
      ...
      <div class="help-body">
        ...
        2. 朱色の「通し練習をスタート」→確認→カウントダウン。スタートから実施中まで「本日何本目」かを大きく表示し、スタートボタンにはこれまでの合計本数も表示します。初期値は5秒で、確認画面の−/＋からルーティンごとに変更できます<br>
        3. 通しがノーミスなら、技一覧の一番下にある「クリーン」を1タップ<br>
        4. 失敗したら、落ちた技をタップ → 種類を選ぶ(初期値: ドロップして中止)。中止なら前の技まで成功・後ろは未到達として自動処理。復帰して続けたら最後に「完走」<br>
        5. 次の1本はもう一度スタート。練習を終えたら「セッション終了」で振り返りと「次回試すこと」をメモします
      </div>
    </div>
    <div class="card">
      <h2>楽曲と録音</h2>
      <div class="help-body">編集画面で音源(MP3等)を添付すると、通し練習中に再生できます。編集画面・通し練習・パート練習の音量は共通で、各画面の音量バーから調整できます。<b>失敗をタップした瞬間の曲位置(♪1:23)が自動で記録</b>され、曲は自動停止。クリーン/中止を記録すると曲は頭に戻るので、次の通しは「▶ 再生」を押すだけです。<br><br>音源は「音源ライブラリ」に貯めて、どのルーティンでも「♪ ライブラリから」で使い回せます。マイク録音した音源もライブラリに追加できます。<br><br>音源はこの端末のブラウザ内にのみ保存され、JSONバックアップには含まれません。</div>
    </div>
    <div class="card">
      <h2>パート練習</h2>
...
function renderSettings() {
  ...
  return `
    ...
    <div class="card">
      <h2>ルーティンで使う機能${infoBtn("editorFeatures")}</h2>
      ${switchRow("リスク度", "各技に危険度(1〜5)を設定・表示します", "showRisk")}
      ${switchRow("A/B分岐", "本番でどちらの技をやるか選べるステップ(A/B)を作れます", "showSlots")}
    </div>
    <div class="card">
...
window.toggleSetting = (key, val) => {
  ...
  toast(`${label}を${val ? "ON" : "OFF"}にしました`);
};
