...
const SAMPLE_HISTORY_SCHEMA = 3;
const SAMPLE_SEQUENCE_SCHEMA = 2;
const SAMPLE_TRANSITION_COLOR_SCHEMA = 1;

const APP_VERSION = "v231"; // 要望フォーム等で自動送信するアプリ版
...
function migrateState() {
  ...
  if (Object.prototype.hasOwnProperty.call(state.settings, "practicePreviewMode")) {
    delete state.settings.practicePreviewMode;
  ...
  for (const rt of state.routines || []) {
    // 右上の「個別設定」はルーティンごとに保持する。
    // 全体一括設定は廃止したため、個別値が無い旧データは任意機能をOFFで補う。
  ...
  for (const rt of state.routines || []) {
    ...
    if (sampleSequenceUpgrade) sampleDataChanged = true;
    if (linkSampleVideos(rt)) sampleDataChanged = true;
    const upgraded = ensureSampleVersionDemo(rt);
    if (ensureSampleTransitionColors(rt)) sampleDataChanged = true;
    const sampleHistoryUpgrade = !!rt.sampleSet && Number(rt.sampleHistorySchema || 0) < SAMPLE_HISTORY_SCHEMA;
    if (upgraded || sampleSequenceUpgrade || sampleHistoryUpgrade) {
    ...
    if (seedSampleHistory(rt, upgraded || sampleSequenceUpgrade || sampleHistoryUpgrade)) sampleDataChanged = true;
  }
...
const pctFine = (x) => {
  const value = Math.round(Number(x || 0) * 1000) / 10;
  return Number.isInteger(value) ? String(value) : value.toFixed(1);
};
const countedFailureEvent = (event) => event && event.type !== "avoid" && event.type !== "not_attempted";
const runFailureEventCount = (run) => (run && Array.isArray(run.events) ? run.events : []).filter(countedFailureEvent).length;
...
function versionStats(routine, versionId) {
  ...
  for (const r of runs) for (const e of r.events) {
    if (!countedFailureEvent(e)) continue;
    fails++;
    if (e.type !== "drop_abort") recov++;
  }
  // 1通しの中で起きる失敗回数を、過去の実測分布として示す。
  // 「回避」「実施できなかった」は別の意思決定を示すため、失敗回数には含めない。
  const failureCounts = runs.map(runFailureEventCount);
  const failureCountDistribution = [
    { key: "zero", n: failureCounts.filter((count) => count === 0).length },
    { key: "one", n: failureCounts.filter((count) => count === 1).length },
    { key: "two", n: failureCounts.filter((count) => count === 2).length },
    { key: "threePlus", n: failureCounts.filter((count) => count >= 3).length },
  ].map((bucket) => ({ ...bucket, rate: total ? bucket.n / total : 0 }));
  const failuresPerRun = total ? fails / total : 0;
  // 内訳: 何本目か / 体調
  ...
  return { ver, runs, total, clean, steps, recov, fails, failuresPerRun, failureCountDistribution, byRunNo, byFeeling, tagCount, excluded };
}
...
function hideSheet() {
  if (runVideoPostCompositionBusy) {
    toast(isEnglish() ? "Video and music are being combined" : "映像と音源を合成中です");
    return;
  }
...
function editorMusicLoadKey() {
  if (!draft || !draft.music) return null;
  return draft._newMusicFile ? `edit:new:${draft._for}` : `edit:${draft._for}`;
}
function editorMusicEndForDraft() {
  const loaded = musicLoadedFor === editorMusicLoadKey() ? musicEffectiveDuration() : 0;
  return loaded || Number(draft && draft.music && draft.music.duration) || 0;
}
...
async function loadEditorMusic() {
  ...
  musicPlayer.addEventListener("loadedmetadata", () => {
    if (view.name !== "edit" || !draft || musicLoadedFor !== key) return;
    syncMusicMetadata();
    render();
  }, { once: true });
  musicLoadedFor = key;
...
function editorPreviewPlayerHtml(hasMusic) {
  const duration = hasMusic ? editorMusicEndForDraft() : 0;
...
function practiceNowDockHtml(editorPlayer = "") {
  ...
  return `<section class="practice-now paused ${editorPlayer ? "has-editor-player" : ""} ${nameOnly ? "name-only" : ""}" id="practice-now" aria-live="polite" aria-atomic="true">
    ...
    ${nameOnly ? "" : `<div class="practice-now-media" id="practice-now-media"><span class="practice-video-empty">動画プレビュー</span></div>`}
  </section>`;
}
...
async function updatePracticeNowUI() {
  ...
  syncEditPreviewButtons();
  if (practicePreviewNameOnly()) return;
...
function runCameraViewportSize() {
  return {
...
function runCameraFrameSize(cap = runCamera) {
  const preview = document.getElementById("run-camera-preview");
  const previewWidth = Number(preview && preview.videoWidth) || 0;
  const previewHeight = Number(preview && preview.videoHeight) || 0;
  if (previewWidth > 0 && previewHeight > 0) return { width: previewWidth, height: previewHeight };
  const track = cap && cap.stream && cap.stream.getVideoTracks ? cap.stream.getVideoTracks()[0] : null;
  const settings = track && typeof track.getSettings === "function" ? track.getSettings() : {};
  return {
    width: Number(settings.width) || Number(cap && cap.frameWidth) || 0,
    height: Number(settings.height) || Number(cap && cap.frameHeight) || 0,
  };
}
function currentRunCameraOrientationState(profileId, cap = runCamera) {
  const profile = runCameraProfile(profileId || (cap && cap.profileId));
  const viewport = runCameraViewportSize();
  const frame = cap && cap.profileId === profile.id ? runCameraFrameSize(cap) : { width: 0, height: 0 };
...
function syncRunCameraOrientationUi(routineId) {
  ...
  const orientation = currentRunCameraOrientationState(profile.id, ready ? runCamera : null);
  const copy = runCameraOrientationCopy(orientation, ready);
...
function runVideoAspect(video) {
  const capturedRatio = Number(video?.captureAspectRatio);
  if (Number.isFinite(capturedRatio) && capturedRatio > 0) return capturedRatio;
...
function clearPendingRunVideo() {
  ...
  runVideoPostCompositionController = null;
  runVideoPostCompositionBusy = false;
...
function updateRunCameraConfirm(routineId, status = "") {
  ...
  if (preview && runCameraReady(routineId)) {
    ...
    preview.addEventListener("resize", updateFrameOrientation);
    preview.play().catch(() => {});
  }
  syncRunCameraOrientationUi(routineId);
}
...
let runCameraOrientationUiRaf = 0;
...
function startRunVideoCapture(routineId) {
  ...
  try {
    ...
    cap.chunks = [];
    rec.ondataavailable = (event) => { if (event.data.size) cap.chunks.push(event.data); };
    rec.start(1000);
    // Web版では録画中の複合Streamを避け、保存時に完成映像を作る。
    cap.audioEmbedded = false;
    cap.audioMode = cap.music ? "linked" : "none";
    cap.recordingAudioDelaySeconds = 0;
    cap.requestedAudioDelaySeconds = preferredRunVideoAudioDelay();
    cap.rec = rec;
    cap.recording = true;
...
async function stopRunVideoCaptureAtMusicStop() {
  ...
  runVideoStopPromise = (async () => {
    ...
    const captureAspectRatio = cap.captureWidth > 0 && cap.captureHeight > 0
      ? cap.captureWidth / cap.captureHeight : profile.ratio;
    stoppedRunVideoCapture = await finalizeRunVideoComposition({
      ...
      cameraProfile: profile.id, aspectRatio: captureAspectRatio, captureAspectRatio,
      captureWidth: cap.captureWidth, captureHeight: cap.captureHeight,
...
async function showRunVideoReview() {
  ...
  pendingRunVideoMusicBlob = musicBlob || null;
  const willCompose = !!(needsLinkedMusic && musicBlob);
  stopRunVideoAudioSync();
...
function showRunVideoCompositionProgress(pending) {
  releaseSheetMedia();
  const duration = Math.max(0, Number(pending && pending.duration) || 0);
  showSheet(`<section class="run-video-compose-progress" aria-labelledby="run-video-compose-title" aria-busy="true">
    <div class="run-video-compose-mark" aria-hidden="true"><span>映</span><i></i><span>♪</span></div>
    <div class="run-video-compose-head">
      <span>${isEnglish() ? "SAVE PROCESS" : "保存処理"}</span>
      <h3 id="run-video-compose-title">${isEnglish() ? "Combining video and music" : "映像と音源を合成中"}</h3>
      <p>${isEnglish()
        ? "The camera video is being rebuilt with the routine music as one file."
        : "カメラ映像へルーティン音源を重ね、1本のファイルへ作り直しています。"}</p>
    </div>
    <div class="run-video-compose-meter">
      <div><output id="run-video-compose-percent">0%</output><span id="run-video-compose-time">0:00.0 / ${fmtTimeFine(duration)}</span></div>
      <progress id="run-video-compose-bar" max="1" value="0" aria-label="${isEnglish() ? "Composition progress" : "合成の進み具合"}"></progress>
    </div>
    <div class="run-video-compose-note"><b>${isEnglish() ? "Please keep this screen open" : "この画面を開いたままお待ちください"}</b>
      <span>${isEnglish()
        ? "Processing can take about as long as the video. On iPhone browsers, processing time and memory use still need device testing."
        : "映像の長さと同程度かかる場合があります。iPhoneブラウザでの処理時間・メモリ使用量は実機検証が必要です。"}</span></div>
    <button type="button" class="btn ghost" onclick="cancelRunVideoPostComposition()">${isEnglish() ? "Cancel" : "合成を中止"}</button>
  </section>`);
}

function updateRunVideoCompositionProgress(progress, current, duration) {
  const ratio = Math.max(0, Math.min(1, Number(progress) || 0));
  const bar = document.getElementById("run-video-compose-bar");
  const percent = document.getElementById("run-video-compose-percent");
  const time = document.getElementById("run-video-compose-time");
  if (bar) bar.value = ratio;
  if (percent) percent.textContent = `${Math.round(ratio * 100)}%`;
  if (time) time.textContent = `${fmtTimeFine(current || 0)} / ${fmtTimeFine(duration || 0)}`;
}

function showRunVideoCompositionFailure(error, replaceId = "") {
  const supportReason = error && error.code ? String(error.code) : "composition-failed";
  const missingMusic = supportReason === "source-missing" || !pendingRunVideoMusicBlob;
  showSheet(`<section class="run-video-compose-error">
    <span>${isEnglish() ? "THE ORIGINAL VIDEO IS SAFE" : "元の映像は残っています"}</span>
    <h3>${isEnglish() ? "Video and music could not be combined" : "映像と音源を合成できませんでした"}</h3>
    <p>${isEnglish()
      ? "You can retry. If it still fails, save the camera video without rebuilding it."
      : "再試行できます。繰り返し失敗する場合も、カメラ映像を作り直さず保存できます。"}</p>
    <small>${isEnglish() ? "Technical code" : "確認用コード"}: ${esc(supportReason)}</small>
    ${missingMusic ? "" : `<button type="button" class="btn primary" onclick="savePendingRunVideo('${esc(replaceId)}')">${isEnglish() ? "Retry composition" : "合成を再試行"}</button>`}
    <button type="button" class="btn ghost" onclick="savePendingRunVideoLinked('${esc(replaceId)}')">${missingMusic
      ? (isEnglish() ? "Save video only" : "映像のみで保存")
      : (isEnglish() ? "Save with linked music" : "別音源同期のまま保存")}</button>
    <button type="button" class="btn ghost" onclick="showRunVideoReview()">${isEnglish() ? "Back to preview" : "映像の確認へ戻る"}</button>
  </section>`);
}

async function persistPendingRunVideo(pending, replaceId = "", message = "") {
  if (!pending || pendingRunVideo !== pending) return false;
  const id = uid();
  if (!(await blobPut(id, pending.blob))) { toast("映像を保存できませんでした"); return false; }
  const video = {
    ...
    captureAspectRatio: pending.captureAspectRatio,
    captureWidth: pending.captureWidth, captureHeight: pending.captureHeight,
  ...
  toast(message || (replaceId ? "通し映像を入れ替えました" : "通し映像を保存しました"));
  return true;
}

window.cancelRunVideoPostComposition = () => {
  if (runVideoPostCompositionController) runVideoPostCompositionController.abort();
};
...
window.savePendingRunVideoLinked = async (replaceId = "", deferred = false) => {
  ...
  if (saveVideoOnly) {
    pending = await finalizeRunVideoComposition({ ...pending, music: null, audioEmbedded: false });
...
window.savePendingRunVideo = async (replaceId = "") => {
  ...
  if (!pendingRunVideoMusicBlob) return showRunVideoCompositionFailure(
    Object.assign(new Error("Music source is missing"), { code: "source-missing" }), replaceId);

  runVideoPostCompositionBusy = true;
  runVideoPostCompositionController = new AbortController();
  showRunVideoCompositionProgress(pending);
  try {
    ...
    if (pendingRunVideo !== pending) return;
    pendingRunVideo = finalizeRunVideoPostComposition(pending, composed);
    runVideoPostCompositionBusy = false;
    runVideoPostCompositionController = null;
    const elapsedSeconds = Math.max(1, Math.round((Number(composed.elapsedMs) || 0) / 1000));
    const completedMessage = isEnglish()
      ? `${replaceId ? "Video replaced" : "Video saved"} after combining music (${elapsedSeconds} sec)`
      : replaceId ? `映像と音源を合成して入れ替えました（処理${elapsedSeconds}秒）`
        : `映像と音源を合成して保存しました（処理${elapsedSeconds}秒）`;
    await persistPendingRunVideo(pendingRunVideo, replaceId,
      completedMessage);
  } catch (error) {
    runVideoPostCompositionBusy = false;
    runVideoPostCompositionController = null;
    ...
    if (error && error.code === "aborted") {
      await showRunVideoReview();
      toast(isEnglish() ? "Composition cancelled" : "合成を中止しました");
      return;
    }
...
window.showVersionRestoreConfirm = (routineId, versionIndex) => {
  ...
  const description = isEnglish()
    ? `The v${versionIndex + 1} sequence will replace the unsaved steps in the editor. The current sequence and practice records will remain. When saving, you can add it as v${nextVersion} or overwrite the current v${rt.versions.length}.`
    : `v${versionIndex + 1}の構成を編集画面へ読み込みます。編集中の未保存ステップは置き換わりますが、現在の構成と練習記録は残ります。保存時にv${nextVersion}として追加するか、現在のv${rt.versions.length}を上書きするか選べます。`;
  showSheet(`
    <h3>${title}</h3>
...
window.loadRoutineVersionIntoDraft = (routineId, versionIndex) => {
  ...
  hideSheet(); render();
  toast(`v${versionIndex + 1}の構成を読み込みました。保存時に保存方法を選べます`);
};
...
function stepDur(s) {
  ...
  // A/Bはどちらを選んでも次のキューへ食い込まないよう、長い選択肢を基準にする。
  if (isSlot(s)) return Math.max(...s.options.map(stepDur));
  if (s && s.trickId) {
    const t = (state.tricks || []).find((x) => x.id === s.trickId);
    const linked = Number(t && t.duration);
...
window.addEventListener("resize", () => {
  if (view.name === "edit") syncEditorSequenceDurations();
});
...
// 現在のシーケンス終端と次のキュー（末尾は楽曲終了）との差。正なら空白、負なら重なり。
function cueIntervalAt(index) {
  const current = draft && draft.steps[index];
  const next = draft && draft.steps[index + 1];
  if (!current || current.cue == null) return null;
  const currentCue = Number(current.cue);
  const terminal = !next && index === draft.steps.length - 1;
  if (!terminal && (!next || next.cue == null)) return null;
  const nextCue = terminal ? editorMusicEndForDraft() : Number(next && next.cue);
  const duration = stepDur(current);
  if (terminal && !(nextCue > 0)) return null;
  if (![currentCue, nextCue, duration].every(Number.isFinite)) return null;
  const delta = Math.round((nextCue - currentCue - duration) * 10) / 10;
  if (Math.abs(delta) < 0.05) return null;
  return {
    delta,
    kind: delta > 0 ? "gap" : "overlap",
    terminal,
    key: [current.id, terminal ? "music-end" : next.id, currentCue.toFixed(1), duration.toFixed(1), nextCue.toFixed(1)].join("|"),
  };
}
function cueIntervalWarningHtml(index) {
  const interval = cueIntervalAt(index);
  if (!interval || draft._dismissedCueIntervals?.has(interval.key)) return "";
  const seconds = Math.abs(interval.delta).toFixed(1);
  const label = isEnglish()
    ? (interval.terminal
        ? (interval.kind === "gap" ? `${seconds}s gap before music ends` : `${seconds}s past music end`)
        : (interval.kind === "gap" ? `${seconds}s gap` : `−${seconds}s negative interval`))
    : (interval.terminal
        ? (interval.kind === "gap" ? `楽曲終了まで ${seconds}秒の空間あり` : `楽曲終了を ${seconds}秒超過`)
        : (interval.kind === "gap" ? `${seconds}秒の空間あり` : `−${seconds}秒のマイナス区間あり`));
    ...
    <div class="editor-step" data-line-color="${itemLineColor(s)}">
        ...
        <div class="es-playback-controls">
          ...
          <span class="cue-position-actions">
            ...
            <button class="mini-btn cue-fit" onclick="fitCueToPrevious(${i})"
              aria-label="${fitLabel}" title="${fitLabel}">FIT</button>
          </span>
      ...
      ${isSlot(s) && showSlots ? s.options.map((o, oi) => `
        <div class="es-opt">
          ...
          <input type="text" value="${esc(optionDisplayName(o))}" placeholder="選択肢${String.fromCharCode(65 + oi)}のシーケンス名"
...
window.fitCueToPrevious = (i) => {
  const s = draft && draft.steps[i];
  if (!s) return;
  if (s.cueLocked) {
    return toast(isEnglish() ? "Unpin this position before using FIT" : "ピンを外すとFITできます");
  }
  let target = 0;
  if (i > 0) {
    const previous = draft.steps[i - 1];
    if (!previous || previous.cue == null || !Number.isFinite(Number(previous.cue))) {
      return toast(isEnglish() ? "Set the previous cue before using FIT" : "前のシーケンスのキューを先に設定してください");
    }
    target = Math.round((Number(previous.cue) + stepDur(previous)) * 10) / 10;
  }
  s.cue = target;
  render();
  toast(isEnglish() ? `Cue fitted to ${fmtCue(target)}` : `キューを ${fmtCue(target)} に合わせました`);
};
window.dismissCueInterval = (i) => {
  const interval = cueIntervalAt(i);
  if (!interval || !draft) return;
  if (!(draft._dismissedCueIntervals instanceof Set)) draft._dismissedCueIntervals = new Set();
  draft._dismissedCueIntervals.add(interval.key);
  render();
};
...
function insertDraftStep(step, insertAt) {
  const at = insertAt == null ? NaN : Number(insertAt);
  if (!draft || !step) return false;
  if (!Number.isInteger(at) || at < 0 || at > draft.steps.length) {
    draft.steps.push(step);
    return false;
  }
  const previous = draft.steps[at - 1];
  const cue = at === 0 ? 0 : (previous && previous.cue != null
    ? Math.round((Number(previous.cue) + stepDur(previous)) * 10) / 10 : null);
  if (Number.isFinite(cue)) step.cue = cue;
  draft.steps.splice(at, 0, step);
  return true;
}
window.addStep = (kind, insertAt = null) => {
  insertDraftStep({ id: uid(), name: "", kind, lineColor: "blue" }, insertAt);
  render();
};
...
window.addStepFromTrick = (trickId, insertAt = null) => {
  const t = (state.tricks || []).find((x) => x.id === trickId);
  if (!t || !draft) return hideSheet();
  insertDraftStep({ id: uid(), name: t.name, kind: "trick", trickId: t.id,
    lineColor: itemLineColor(t), sampleContent: !!t.sample }, insertAt);
  hideSheet(); render();
...
function showRoutineSaveChoice(rt) {
  const currentVersion = latestVersion(rt);
  const currentNo = rt.versions.length;
  const nextNo = currentNo + 1;
  const runCount = runsOfVersion(rt.id, currentVersion.id).length;
  const restoredNote = draft._restoredFromVersion
    ? (isEnglish()
        ? `The editor currently contains the sequence loaded from v${draft._restoredFromVersion}.`
        : `現在はv${draft._restoredFromVersion}から読み込んだ構成を編集中です。`)
    : "";
  const versionCopy = isEnglish()
    ? `Saving as v${nextNo} keeps v${currentNo} and its analysis unchanged. Future practice will use v${nextNo}.`
    : `v${nextNo}として保存すると、v${currentNo}の構成と分析をそのまま残せます。次回からはv${nextNo}で練習します。`;
  const overwriteCopy = runCount
    ? (isEnglish()
        ? `Current v${currentNo} has ${runCount} full-run record${runCount === 1 ? "" : "s"}. Overwriting replaces its sequence, so those records will also be shown against the edited sequence.`
        : `現在のv${currentNo}には通し${runCount}本の記録があります。上書きすると構成が置き換わり、それらの分析も修正後の構成に対して表示されます。`)
    : (isEnglish()
        ? `Overwriting replaces the current v${currentNo} sequence.`
        : `上書きすると、現在のv${currentNo}の構成を置き換えます。`);
  showSheet(`
    <h3>${isEnglish() ? "Choose how to save" : "保存方法を選ぶ"}</h3>
    <div class="sheet-sub" data-user-text>${esc(routineDisplayName(rt))}</div>
    <div class="version-restore-summary">
      <b>v${currentNo} → v${nextNo}</b>
      <span>${isEnglish() ? `${draft.steps.length} sequences` : `${draft.steps.length}シーケンス`}</span>
    </div>
    ${restoredNote ? `<p class="version-restore-copy">${restoredNote}</p>` : ""}
    <p class="version-restore-copy">${versionCopy}</p>
    <button class="btn primary" onclick="commitRoutineSave('version')">${isEnglish()
      ? `Save as new v${nextNo}${runCount ? " (recommended)" : ""}`
      : `新しいv${nextNo}として保存${runCount ? "（推奨）" : ""}`}</button>
    <p class="version-restore-copy">${overwriteCopy}</p>
    <button class="btn danger-ghost" style="width:100%" onclick="commitRoutineSave('overwrite')">${isEnglish()
      ? `Overwrite current v${currentNo}`
      : `現在のv${currentNo}を上書き保存`}</button>
    <p class="hint">${isEnglish()
      ? "This choice applies to the sequence. Routine name, music, and Routine Settings are updated either way."
      : "この選択はシーケンス構成の保存方法です。ルーティン名・楽曲・個別設定は、どちらを選んでも現在の内容に更新されます。"}</p>
    <button class="btn ghost" onclick="hideSheet()">${isEnglish() ? "Cancel" : "キャンセル"}</button>`);
}

let routineSavePending = false;
window.commitRoutineSave = async (mode) => {
  if (!draft || routineSavePending) return;
  const rt = draft.id ? state.routines.find((routine) => routine.id === draft.id) : null;
  if (rt && mode !== "version" && mode !== "overwrite") return;
  if (!rt && mode !== "new") return;
  hideSheet();
  routineSavePending = true;
  try {
    return await withLoading("ルーティンを保存中…", async () => {
      let savedMessage = "";
      if (rt) {
        ...
        if (musicLoadedFor === rt.id) musicLoadedFor = null;
        if (mode === "version") {
          ...
          savedMessage = isEnglish()
            ? `Saved as v${rt.versions.length}. Analysis remains separated by version.`
            : `v${rt.versions.length}として保存しました。分析はバージョン別に残ります`;
        } else {
          const currentNo = rt.versions.length;
          const current = latestVersion(rt);
          current.steps = cloneRoutineSteps(draft.steps);
          current.createdAt = Date.now();
          savedMessage = isEnglish()
            ? `Overwrote v${currentNo}.`
            : `v${currentNo}を上書き保存しました`;
        }
      } else {
        ...
        state.routines.push({ id: uid(), name: draft.name.trim(), music, lineColor: "blue",
          ...
          versions: [{ id: uid(), createdAt: Date.now(), steps: cloneRoutineSteps(draft.steps) }] });
        savedMessage = isEnglish() ? "Saved as v1." : "v1として保存しました";
      }
      saveState(); draft = null; go("routines"); toast(savedMessage);
    });
  } finally {
    routineSavePending = false;
  }
};
...
window.saveRoutine = async () => {
  ...
  if (draft.id) {
    const rt = state.routines.find((routine) => routine.id === draft.id);
    if (!rt) return toast("ルーティンが見つかりません");
    return showRoutineSaveChoice(rt);
  }
  return window.commitRoutineSave("new");
};
...
window.confirmRunStart = (routineId) => {
  ...
  showSheet(`
    ...
    <button class="btn primary run-confirm-btn" id="run-confirm-start" onclick="startRunCountdown('${rt.id}')">始める</button>
    <button class="btn ghost" onclick="hideSheet()">キャンセル</button>`);
};
...
// ========== 統計 ==========
function failureRateClass(item) {
  const attempted = Number(item && (item.attempted == null ? item.reached : item.attempted));
  const rate = attempted > 0 ? Number(item.failed || 0) / attempted : 0;
  if (rate >= 0.5) return "failure-rate-red";
  if (rate >= 0.3) return "failure-rate-orange";
  if (rate >= 0.1) return "failure-rate-yellow";
  return "";
}
function renderStats() {
  ...
  const english = isEnglish();
  const overview = `
    <div class="stat-overview analysis-overview">
      <div class="stat-box"><div class="v">${st.total}</div><div class="l">通し数</div></div>
      <div class="stat-box"><div class="v">${st.clean}/${st.total}</div><div class="l">クリーン率 ${pct(st.clean / st.total)}%</div></div>
    </div>`;
  const failureCountLabels = english
    ? { zero: "0 issues", one: "1 issue", two: "2 issues", threePlus: "3+ issues" }
    : { zero: "0回", one: "1回", two: "2回", threePlus: "3回以上" };
  const failureCountEstimate = `
    <section class="failure-count-estimate" aria-label="${english ? "Average mistakes" : "平均ミス回数の目安"}">
      <div class="failure-count-head">
        <div><span>${english ? "ESTIMATE" : "確率の目安"}</span><h2>${english ? "Average mistakes" : "平均ミス回数"}</h2></div>
        <b>${english ? `Average ${st.failuresPerRun.toFixed(1)}` : `平均 ${st.failuresPerRun.toFixed(1)}回`}</b>
      </div>
      <div class="failure-count-grid">
        ${st.failureCountDistribution.map((bucket) => `<div class="failure-count-cell">
          <strong>${pctFine(bucket.rate)}%</strong><span>${failureCountLabels[bucket.key]}</span>
        </div>`).join("")}
      </div>
      <p>${english
        ? "Estimated from past full runs. Skips and unattempted sequences are excluded."
        : "これまでの通しから推定。回避・実施できずは含みません。"}</p>
    </section>`;

  // ステップ一覧は「実施した中での失敗」と「到達したが実施できなかった」を分ける。
  // 到達数、事前リスク、曲位置などの補助パラメータは詳細画面で扱う。
  ...
  const stepRows = st.steps.map((s) => {
    ...
    if (s.options && showSlots) {
      ...
      const optRows = s.options.map((o) => {
        return `<div class="slot-opt-stat ${failureRateClass(o)}">
          <div class="head"><span class="nm">└ ${esc(optionDisplayName(o.opt))}</span>
            <span class="kn">${failSummary(o)}</span></div>
        </div>`;
      }).join("");
      return `<div class="step-stat ${s.step.kind} ${failureRateClass(s)}" ${openDetail}>
  ...
  return `
    ...
    ${verSelect}
    ${overview}
    ${failureCountEstimate}
    <div class="card">
      <h2>ステップ別の失敗・未実施</h2>
      <div class="failure-rate-legend" aria-label="${isEnglish() ? "Issue rate color thresholds" : "失敗率の色分け"}">
        <span class="failure-rate-yellow">${isEnglish() ? "10%+" : "10%〜"}</span>
        <span class="failure-rate-orange">${isEnglish() ? "30%+" : "30%〜"}</span>
        <span class="failure-rate-red">${isEnglish() ? "50%+" : "50%〜"}</span>
      </div>
      ${stepRows}
    </div>
    ${runVideoCard}
...
function partLoopDelaySeconds(rt) {
  const stored = rt && rt.partLoop ? rt.partLoop.delaySeconds : null;
  if (stored == null) return PART_LOOP_DELAY_DEFAULT;
  const raw = Number(stored);
  if (!Number.isFinite(raw)) return PART_LOOP_DELAY_DEFAULT;
...
window.partClear = () => {
  ...
  if (rt.partLoop) {
    ...
    if (!Object.prototype.hasOwnProperty.call(rt.partLoop, "delaySeconds")) delete rt.partLoop;
  }
  saveState(); render();
};
window.partChangeLoopDelay = (delta) => {
  ...
  // 未設定は3秒、明示的に「すぐ」を選んだ場合は0秒として区別して保存する。
  rt.partLoop.delaySeconds = next;
...
function sampleTrickId(name) {
  ...
  const trick = (state.tricks || []).find((item) => item.sample
    && String(item.name || "").replace(/\s*\(サンプル\)$/, "").startsWith(normalized));
  return trick && trick.id;
}
function linkSampleVideos(rt) {
  if (!rt?.sampleSet) return false;
  let changed = false;
  for (const version of rt.versions || []) {
    for (const step of version.steps || []) {
      if (!isSlot(step) && step.kind === "trick" && !step.trickId) {
        const trickId = sampleTrickId(step.name);
        if (trickId) {
          step.trickId = trickId;
          changed = true;
        }
      }
      for (const option of step.options || []) {
        if (option.trickId) continue;
        const trickId = sampleTrickId(option.name);
        if (trickId) {
...
function remapExpandedSampleSessions(rt, version, previousSteps) {
  const nextIndex = new Map((version.steps || []).map((step, index) => [step.id, index]));
  for (const session of state.sessions || []) {
    if (session.routineId !== rt.id || session.versionId !== version.id || session.sampleHistory) continue;
    for (const run of session.runs || []) {
      for (const event of run.events || []) {
        const stepId = event.stepId || previousSteps[event.stepIndex]?.id;
        if (stepId && nextIndex.has(stepId)) {
          event.stepId = stepId;
          event.stepIndex = nextIndex.get(stepId);
        }
      }
      const reachedId = previousSteps[run.reachedIndex]?.id;
      if (reachedId && nextIndex.has(reachedId)) run.reachedIndex = nextIndex.get(reachedId);
    }
  }
}
function ensureSampleSequenceDemo(rt) {
  if (!rt?.sampleSet || Number(rt.sampleSequenceSchema || 0) >= SAMPLE_SEQUENCE_SCHEMA) return false;
  const version = latestVersion(rt);
  if (!version || !Array.isArray(version.steps)) return false;
  const previousSteps = version.steps.slice();
  const additions = [
  ...
  for (const addition of additions) {
    if (version.steps.some((step) => String(step.name || "") === addition.name)) continue;
    const step = {
      id: uid(), name: addition.name, kind: addition.kind, risk: addition.risk, cue: addition.cue,
      trickId: sampleTrickId(addition.name), lineColor: "blue", sampleContent: true,
    };
    let index = version.steps.findIndex(addition.before);
    if (index < 0) index = version.steps.findIndex((item) => String(item.name || "").includes("フィニッシュ"));
    version.steps.splice(index < 0 ? version.steps.length : index, 0, step);
    stepsAdded = true;
  }
...
function ensureSampleTransitionColors(rt) {
  if (!rt?.sampleSet || Number(rt.sampleTransitionColorSchema || 0) >= SAMPLE_TRANSITION_COLOR_SCHEMA) return false;
  let changed = false;
  for (const version of rt.versions || []) {
    for (const step of version.steps || []) {
      const color = step.kind === "transition" ? "rust" : "blue";
      if (step.lineColor !== color) {
        step.lineColor = color;
        changed = true;
      }
    }
  }
  rt.sampleTransitionColorSchema = SAMPLE_TRANSITION_COLOR_SCHEMA;
  return true;
}
...
function seedSampleHistory(rt, force = false) {
  ...
  plans.forEach((plan, sessionIndex) => {
    ...
    const runs = Array.from({ length: 8 }, (_, index) => {
      ...
      const failures = Array.isArray(fail[0]) ? fail : [fail];
      const events = failures.map(([stepKey, type, tags, note], eventIndex) => {
        const stepIndex = sampleFailureStepIndex(ver, stepKey);
        const step = ver.steps[stepIndex];
        const event = {
          stepId: step.id, stepIndex, type, tags, note,
          musicTime: Math.round(((Number(step.cue) || 0) + 0.6 + (runNo % 3) * 0.2 + eventIndex * 0.1) * 10) / 10,
        };
        if (stepIndex === slotIndex && chosen) event.optionId = chosen.id;
        return event;
      });
      const abortEvent = events.find((event) => event.type === "drop_abort");
      return {
        id: uid(), at: startedAt + runNo * 7 * 60 * 1000,
        outcome: abortEvent ? "aborted" : "finished",
        events, reachedIndex: abortEvent ? abortEvent.stepIndex : ver.steps.length - 1, choices,
      };
    });
    state.sessions.push({
  ...
  rt.sampleHistorySeeded = true;
  rt.sampleHistorySchema = SAMPLE_HISTORY_SCHEMA;
  return true;
}
...
window.loadSampleSet = async () => {
  ...
  steps.forEach((step) => {
    ...
    step.lineColor = step.kind === "transition" ? "rust" : "blue";
...
function renderSettings() {
  ...
  return `
    ...
    <div class="app-copyright">© 2026 ARATA URAWA</div>`;
}
