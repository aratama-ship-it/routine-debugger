...
const APP_VERSION = "v152"; // 要望フォーム等で自動送信するアプリ版
...
function runCameraConfirmBody(routineId, status = "") {
  ...
  return `
    <div class="run-camera-head">
      <div><b>インカメで撮影</b><span>音声なし・通し終了まで</span></div>
      <span class="run-video-capacity">保存 ${count}/${RUN_VIDEO_LIMIT}本</span>
    </div>
    ${ready ? `<video id="run-camera-preview" class="run-camera-preview" autoplay playsinline muted></video>` : ""}
    ...
    <small>映像は端末内に保存します。6本目は保存時に入れ替える映像を選びます。</small>`;
}
...
window.toggleRunCamera = async (routineId) => {
  ...
  try {
    ...
    runCamera = { routineId, stream, chunks: [], recording: false };
    updateRunCameraConfirm(routineId, "インカメの準備ができました");
  } catch (_) {
    if (requestGeneration !== runCameraRequestGeneration) return;
    runCamera = null;
...
async function stopRunVideoCapture(rt, sess, run) {
  ...
  const duration = Math.max(0, (Date.now() - cap.startedAt) / 1000);
  clearPendingRunVideo();
  pendingRunVideo = {
    blob, duration, at: run.at, routineId: rt.id, sessionId: sess.id, runId: run.id,
...
function showRunVideoReview() {
  ...
  showSheet(`
    <h3>通し練習の映像</h3>
    <div class="sheet-sub">音声なし / ${fmtTimeFine(pendingRunVideo.duration)} / ${fmtBytes(pendingRunVideo.size)}</div>
    <video class="run-video-review" src="${pendingRunVideoUrl}" controls playsinline></video>
...
window.openRunVideo = async (id) => {
  ...
  showSheet(`
    ...
    <div class="sheet-sub">インカメ / 音声なし / ${fmtTimeFine(video.duration)}</div>
    <video id="run-video-player" class="run-video-review" src="${sheetVideoUrl}" controls playsinline></video>
...
window.startRunCountdown = (routineId) => {
  ...
  const begin = () => {
    ...
    runCountdownFinishTimer = setTimeout(() => {
      runCountdownFinishTimer = null;
      removeRunCountdownOverlay();
      startRunVideoCapture(routineId);
      render();
      if (rt.music && musicPlayer.src && !musicMissing) {
...
function renderRecord() {
  ...
  return `
    ...
    ${runCamera && runCamera.recording ? `<div class="run-video-live" role="status" aria-live="polite">
      <span class="run-video-live-dot"></span><b>REC</b><span id="run-video-elapsed">${fmtTimeFine((Date.now() - runCamera.startedAt) / 1000)}</span>
      <small>インカメ・音声なし</small>
    </div>` : ""}
    ${musicCard}
...
function renderHelpEnglish() {
  return `
    ...
    <div class="card"><h2>Full-run workflow</h2>
      ...
      2. Tap the large vermilion start button, confirm the countdown, and begin. You can change the countdown with − / + on the confirmation screen. To film the run, prepare the front camera there before starting. Recording is video-only and stops with the run.<br>
  ...
  return `
    ...
    <div class="card">
      ...
      <div class="help-body">
        ...
        2. 朱色の「通し練習をスタート」→確認→カウントダウン。スタートから実施中まで「本日何本目」かを大きく表示し、スタートボタンにはこれまでの合計本数も表示します。初期値は5秒で、確認画面の−/＋からルーティンごとに変更できます。映像を残す場合は、同じ確認画面で「インカメを準備」を押します<br>
