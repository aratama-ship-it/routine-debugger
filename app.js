...
const APP_VERSION = "v163"; // 要望フォーム等で自動送信するアプリ版
...
async function showRunVideoReview() {
  ...
  showSheet(`
    <h3>通し練習の映像</h3>
    <div class="sheet-sub">${uiText(profile.label)} / ${isEnglish() ? "Recorded without audio" : "撮影音声なし"} / ${fmtTimeFine(pending.duration)} / ${fmtBytes(pending.size)}</div>
    ...
    return `<article class="run-video-library-row">
      ...
      <div class="run-video-library-copy">
        ...
        <small>${fmtTimeFine(video.duration)} / ${fmtBytes(video.size || 0)} / ${linkedMusic
          ? (english ? `Music sync: ${esc(linkedMusic.name || "Linked music")}` : `♪ ${esc(linkedMusic.name || "対象音源")}と同期`)
      ...
      ${runVideos.map((video) => `
        <div class="run-video-row">
          <div><b>${esc(runVideoTitle(video))}</b><span>${fmtTimeFine(video.duration)} / ${runVideoMusicMeta(video)
            ? `♪ ${esc(runVideoMusicMeta(video).name || "対象音源")}と同期`
            : "映像のみ"}</span></div>
...
function renderHelpEnglish() {
  return `
    ...
    <div class="card"><h2>Full-run workflow</h2>
      ...
      2. Tap the large vermilion start button, confirm the countdown, and begin. You can change the countdown with − / + on the confirmation screen. To film the run, choose 4:3 Landscape or 9:16 Tall and prepare the front camera there. The setup preview uses the selected framing. Recording is video-only, starts with music playback, and stops when the music stops.<br>
