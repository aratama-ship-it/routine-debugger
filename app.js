...
const APP_VERSION = "v153"; // 要望フォーム等で自動送信するアプリ版
...
function runCameraConfirmBody(routineId, status = "") {
  ...
  return `
    ...
    <small>映像は端末内に保存します。撮影中はREC欄にプレビューを表示し、6本目は保存時に入れ替える映像を選びます。</small>`;
}
...
function renderRecord() {
  ...
  return `
    ...
    ${runCamera && runCamera.recording ? `<div class="run-video-live" role="status" aria-live="polite">
      <video id="run-camera-live-preview" class="run-camera-live-preview" autoplay playsinline muted
