...
const APP_VERSION = "v111"; // 要望フォーム等で自動送信するアプリ版
...
function practiceStepName(rt, step) {
  ...
  if (!routineFeatureEnabled(rt, "showSlots")) return (step.options[0] && step.options[0].name) || step.name || "名称未設定";
...
function editorPreviewPlayerHtml(hasMusic) {
  ...
  return `<div class="practice-now-player ${hasMusic ? "" : "is-disabled"}" aria-label="楽曲プレイヤー">
    ...
    <div class="practice-now-player-main">
    ...
    <div class="practice-now-player-actions">
...
window.showRoutineMenu = (routineId) => {
  ...
  const routineName = rt ? rt.name : (draft && draft.name ? draft.name : "新規ルーティン");
  showSheet(`
    <h3>個別設定</h3>
...
function routineCardHtml(rt, context = "list") {
  ...
  const deleteLabel = isEnglish() ? `Delete ${rt.name}` : `${rt.name}を削除`;
  const memo = String(rt.memo || "").trim();
  const memoLabel = isEnglish()
    ? (memo ? `Open full memo for ${rt.name}` : `Add a quick memo to ${rt.name}`)
    : (memo ? `${rt.name}の簡易メモ全文を開く` : `${rt.name}に簡易メモを追加`);
  const memoHtml = `
      <div class="routine-quick-note ${memo ? "" : "empty"}" role="button" tabindex="0"
        ...
        title="${memo ? "タップして全文を表示" : "タップしてメモを追加"}">
...
window.showRoutineMemo = (id) => {
  ...
  if (!rt) return;
  const memo = String(rt.memo || "").trim();
  if (!memo) return showRoutineMemoEditor(id);
  showSheet(`
    <h3>簡易メモ</h3>
    <div class="sheet-sub" data-user-text>${esc(rt.name)}</div>
    <div class="routine-memo-full" data-user-text>${esc(memo)}</div>
...
window.showRoutineMemoEditor = (id) => {
  ...
  showSheet(`
    ...
    <textarea id="routine-memo-input" rows="7" maxlength="1000"
      placeholder="次回試したいこと、衣装・道具の注意点など">${esc(rt.memo || "")}</textarea>
...
window.showDeleteRoutine = (id) => {
  ...
  const detailText = isEnglish()
    ? "The routine, its practice records, recordings, and attached music will be removed."
    : "ルーティン、練習記録、録音、添付した楽曲が削除されます。";
  showSheet(`
    <h3>ルーティンを削除</h3>
    <div class="delete-routine-name" data-user-text>${esc(rt.name)}</div>
...
function deleteSlideState(track, handle, id) {
  ...
  return { id, track, handle, fill, copy, rect, inset, max, progress: Number(handle.dataset.progress || 0) };
}
...
function finishRoutineDeleteSlide(event, cancelled = false) {
  ...
  if (!cancelled && d.progress >= .94) performRoutineDelete(d.id);
...
window.routineDeleteKey = (event, id) => {
  const track = event.currentTarget;
  const handle = track.querySelector(".delete-slide-handle");
  if (!handle) return;
  const d = deleteSlideState(track, handle, id);
  let next = d.progress;
  if (event.key === "ArrowRight") next += .1;
    ...
    event.preventDefault(); performRoutineDelete(id); return;
  } else return;
  event.preventDefault();
...
function renderEdit() {
        ...
        <div class="es-playback-controls">
          ...
          ${s.trickId && (state.tricks || []).some((t) => t.id === s.trickId)
            ? `<button class="mini-btn video-trim-btn" aria-label="動画を再生・トリム" title="動画を再生・トリム"
                 onclick="sheetTrimTrick('${s.trickId}')">▶</button>`
        ...
        <div class="es-opt">
          ...
          ${o.trickId && (state.tricks || []).some((t) => t.id === o.trickId)
            ? `<button class="mini-btn video-trim-btn" aria-label="選択肢の動画を再生・トリム" title="動画を再生・トリム"
                 onclick="sheetTrimTrick('${o.trickId}')">▶</button>`
  ...
  // 曲のシークと同じ幅・同じ起点に揃える構成タイムライン。二つの時間軸を上下で見比べる。
  let tlBar = "";
  if (hasEditorMusic && draft.steps.length) {
    ...
    tlBar = `
      <div class="music-track-line timeline-track-line">
        <span class="music-track-label">構成</span>
  ...
  return `
    ...
    <div class="card">
      ...
      <p class="hint editor-pin-hint">ピンを打つと、並べ替えや自動セットでもその技の曲位置を維持します。</p>
    </div>
...
function renderStats() {
  ...
  // ステップ一覧は判断に必要な「失敗回数」と「失敗率」だけを主表示にする。
  // 到達数、95%区間、事前リスク、曲位置などの補助パラメータはこの一覧へ重ねない。
  const failSummary = (item) => {
    const rate = item.reached ? item.failed / item.reached : 0;
    return isEnglish()
      ? `${item.failed} issues · ${pct(rate)}%`
      : `失敗 ${item.failed}回・${pct(rate)}%`;
  };
...
function renderStepDetail() {
  ...
  const failRuns = runs.filter((r) => r.events.some((e) => e.stepIndex === i)).length;
  const failRate = reached ? failRuns / reached : 0;
  ...
  const optBreakdown = isSlot(step) && showSlots ? step.options.map((o) => {
    ...
    const oRate = oReached ? oFailed / oReached : 0;
    return `<div class="bd-row"><span class="k">${esc(o.name)}</span><span class="v">${isEnglish() ? `${oFailed} issues · ${pct(oRate)}%` : `失敗 ${oFailed}回・${pct(oRate)}%`}</span></div>`;
  }).join("") : "";
  ...
  return `
    ...
    <div class="stat-overview" style="grid-template-columns:1fr 1fr">
      <div class="stat-box"><div class="v">${isEnglish() ? failRuns : `${failRuns}回`}</div><div class="l">失敗回数</div></div>
      <div class="stat-box"><div class="v">${pct(failRate)}%</div><div class="l">失敗率</div></div>
    </div>
  ...
  const rows = tricks.map((t) => `
    <div class="trick-row">
      <div class="head">
        ...
        <button class="mini-btn video-trim-btn" aria-label="${esc(t.name)}の動画を再生・トリム" title="動画を再生・トリム" onclick="sheetTrimTrick('${t.id}')">▶</button>
...
window.sheetLinkTrickToOption = (i, oi) => {
  ...
  const optionLabel = option.name || `選択肢${String.fromCharCode(65 + oi)}`;
  showSheet(`
...
window.linkTrickToOption = (i, oi, trickId) => {
  ...
  if (!option.name.trim()) option.name = t.name.replace(/\s*\(サンプル\)$/, "");
  hideSheet(); render();
...
function renderHelp() {
  ...
  return `
    ...
    <div class="card">
      <h2>このアプリ</h2>
      <div class="help-body">通し練習を「クリーン1タップ / 失敗1タップ」で記録して、ルーティンの<b>どこで落ちるか</b>の偏りを見るためのアプリです。ルーティン一覧の右上にある✕から確認画面を開き、最後までスライドした場合だけ削除できます。</div>
    </div>
    <div class="card">
      <h2>通し練習の流れ</h2>
      <div class="help-body">
