...
const APP_VERSION = "v167"; // 要望フォーム等で自動送信するアプリ版
...
function runCameraConfirmBody(routineId, status = "") {
  ...
  return `
    ...
    <small>${routine && routine.music
      ? (isEnglish()
        ? "The selected framing is used throughout. The app music is digitally recorded in the video; the camera microphone is not used."
        : "選んだ画角を確認映像・撮影中・保存後まで維持します。アプリ音源を映像へデジタル収録し、カメラのマイク音は入れません。")
      : (isEnglish()
