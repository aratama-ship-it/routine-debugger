...
const APP_VERSION = "v31"; // 要望フォーム等で自動送信するアプリ版
...
function renderEdit() {
  ...
  // リスク度はプルダウン(省スペース)。選択値で文字色を変えて危険度が一目で分かるようにする
  const riskSelect = (selected, onchangeTpl) => `
    <select class="risk-select risk-${selected}" onchange="${onchangeTpl}" aria-label="リスク度">
  ...
  const stepRows = draft.steps.map((s, i) => `
    <div class="editor-step">
      ...
      <div class="es-row2">
        ...
        ${!isSlot(s) ? riskSelect(s.risk || 3, `setRisk(${i},this.value)`) : ""}
      ...
      ${isSlot(s) ? s.options.map((o, oi) => `
        <div class="es-opt">
          ...
          ${riskSelect(o.risk || 3, `setOptRisk(${i},${oi},this.value)`)}
...
window.setRisk = (i, n) => { draft.steps[i].risk = Number(n); render(); };
...
let stepDrag = null;
function setDropIndicator(steps, insertAt) {
  steps.forEach((el, k) => {
    el.classList.toggle("drop-before", k === insertAt);
    el.classList.toggle("drop-after", insertAt === steps.length && k === steps.length - 1);
  });
}
function clearDropIndicator(steps) {
  steps.forEach((el) => el.classList.remove("drop-before", "drop-after"));
}
document.addEventListener("pointerdown", (e) => {
...
window.setOptRisk = (i, oi, n) => { draft.steps[i].options[oi].risk = Number(n); render(); };
