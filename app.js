...
const APP_VERSION = "v52"; // 要望フォーム等で自動送信するアプリ版
...
const INFO = {
  timeline: { t: "タイムライン", b: "各技の長さ(動画リンクは自動、移行は±で調整)を足して曲位置を計算します。「♪ この長さで曲位置を自動セット」で全ステップの♪キューに反映されます。上のプレイヤーで再生するとバーに現在位置が出ます。" },
...
window.editorStepDurAdj = (i, delta) => {
  const s = draft.steps[i]; if (!s) return;
  s.dur = Math.max(0.5, Math.round((stepDur(s) + delta) * 10) / 10);
  render();
};
...
function renderEdit() {
  ...
  // タイムライン(旧「タイムラインで組む」を統合): 各技の長さで曲位置を計算・可視化。音源添付時のみ
  let timelineCard = "";
  if (hasEditorMusic && draft.steps.length) {
    ...
    const durRows = draft.steps.map((s, i) => `
      <div class="bi-row">
        <span class="bi-time">♪${fmtTime(starts[i])}</span>
        <span class="nm">${s.trickId ? "" : "␣ "}${esc(stepLabel(s) || "(無題)")}</span>
        <span class="kn">${stepDur(s).toFixed(1)}s</span>
        <button class="mini-btn" onclick="editorStepDurAdj(${i},-0.5)">−</button>
        <button class="mini-btn" onclick="editorStepDurAdj(${i},0.5)">＋</button>
      </div>`).join("");
    timelineCard = `
    <div class="card">
      <h2>タイムライン (合計 ${fmtTime(total)}${songDur ? ` / 曲 ${fmtTime(songDur)}` : ""})${over ? ` <span style="color:var(--danger)">曲より長い</span>` : ""}${infoBtn("timeline")}</h2>
      <div class="tl-bar">${segs}<div id="edit-tl-playhead"></div></div>
      ${durRows}
      <button class="btn small primary" style="margin-top:10px" onclick="editorAutoCue()">♪ この長さで曲位置を自動セット</button>
    </div>`;
  }
  return `
