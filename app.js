...
const APP_VERSION = "v67"; // 要望フォーム等で自動送信するアプリ版
...
async function updatePracticeNowUI() {
  ...
  if (!current) {
    ...
    const media = document.getElementById("practice-now-media");
    if (name) name.textContent = "技を追加するとここに表示されます";
    if (meta) meta.textContent = "プレビュー位置は固定されます";
    if (media) media.innerHTML = `<span class="practice-video-empty">動画プレビュー</span>`;
  ...
  const name = document.getElementById("practice-now-name");
  const meta = document.getElementById("practice-now-meta");
  if (label) label.textContent = isEdit && editPreviewManual
  ...
  if (meta) meta.textContent = current.next
  ...
  if (!media) return;
  if (!trick) { media.innerHTML = `<span class="practice-video-empty">動画なし</span>`; return; }
  ...
  if (practiceVideoFailed.has(trick.id)) {
    media.innerHTML = `<span class="practice-video-loading">動画を読み込めません</span>`;
    return;
  }
  media.innerHTML = `<span class="practice-video-loading">動画を準備中…</span>`;
...
const INFO = {
  ...
  theme: { t: "デザイン(世界観)", b: "選ぶと編集/練習/分析の配色がこのテーマになります。演目の雰囲気に合わせてどうぞ。" },
  ...
  editorFeatures: { t: "編集画面で使う機能", b: "人によっては使わない機能を、初期状態では隠しています。<br><br><b>リスク度</b> = 各技に危険度(1〜5)を付けて、分析で失敗率とのズレを見る機能。<br><b>A/B分岐</b> = 本番でどちらの技をやるか選べるステップを作る機能。<br><br>必要な人だけONにしてください。ONにすると編集画面(新規作成も同じ)に該当のボタンが出ます。OFFにしても、すでに設定済みのリスク度やA/B分岐のデータは消えません。" },
...
function activeThemeKey() {
  ...
  if (["record", "stats", "part", "stepdetail", "history"].includes(v)) {
    const rt = state.routines.find((r) => r.id === view.params.id);
...
// ルーティン画面共通の右上メニュー。演目のスキン変更を編集画面の奥に置かず、
// 練習・分析・履歴のどこからでも同じ場所で切り替えられるようにする。
function routineMenuAction(routineId, before = "") {
  return `<div class="routine-head-actions">${before}<button class="routine-menu-btn"
    onclick="showRoutineSkinMenu('${routineId || ""}')" aria-label="ルーティンのスキンを変更" title="スキン">
    <span></span><span></span><span></span>
  </button></div>`;
}
window.showRoutineSkinMenu = (routineId) => {
  const editingDraft = view.name === "edit" && draft;
  const rt = routineId ? state.routines.find((r) => r.id === routineId) : null;
  const current = editingDraft ? (draft.theme || "default") : ((rt && rt.theme) || "default");
  showSheet(`
    <h3>ルーティンのスキン</h3>
    <div class="sheet-sub">このルーティンだけの見た目を選べます</div>
    <div class="theme-grid theme-grid-sheet">
      ${THEMES.map((t) => `<button class="theme-swatch ${current === t.key ? "selected" : ""}"
        onclick="chooseRoutineSkin('${routineId || ""}','${t.key}')" aria-label="${t.name}">
...
window.chooseRoutineSkin = (routineId, key) => {
  const theme = isTheme(key) ? key : "default";
  if (view.name === "edit" && draft) {
    draft.theme = theme;
  } else {
  ...
  hideSheet(); render(); toast("スキンを変更しました");
};
    ...
    <div class="home-simple-shell">
      ...
      <footer class="home-libraries" aria-label="ライブラリ">
        <div class="home-library-grid">
          <button onclick="go('tricks')">
            <span class="home-library-copy"><b>技ライブラリ</b><small>${trickCount ? `${trickCount}本` : "未登録"}</small></span>
            <span class="home-button-arrow" aria-hidden="true">›</span>
          </button>
          <button onclick="go('audios')">
            <span class="home-library-copy"><b>音源ライブラリ</b><small>${audioCount ? `${audioCount}件` : "未登録"}</small></span>
            <span class="home-button-arrow" aria-hidden="true">›</span>
          </button>
        </div>
      </footer>
...
window.editorAutoCue = () => {
  ...
  if (!confirm("各技の長さから曲位置(♪)を自動計算して、全ステップのキューを上書きします。よいですか?")) return;
    ...
    <div class="card music-card">
      ...
      <div class="music-foot">
        ...
        <div class="music-foot-actions">
          <button class="mini-btn" onclick="sheetTrimRoutineMusic()">長さ</button>
...
window.duplicateRoutine = async (id) => {
  ...
  state.routines.push({
    id: uid(), name: `${src.name} (コピー)`, music, theme: src.theme || "default", copiedFrom: src.id,
...
function renderRecord() {
  ...
  return `
    <div class="topbar"><button class="back-btn" onclick="endSessionAsk('${rt.id}')">戻る</button>
      <h1>${esc(rt.name)}</h1>${routineMenuAction(rt.id, `<span class="sub">v${rt.versions.length}</span>`)}</div>
    ${practiceNowDockHtml()}
    <div class="runbar">
...
window.discardSession = async (routineId) => {
  ...
  if (n > 0 && !confirm(`このセッションを記録せず破棄します。\n今日の通し ${n} 本は保存されません。よいですか?`)) return;
...
function renderStepDetail() {
  ...
  return `
    <div class="topbar"><button class="back-btn" onclick="go('stats',{id:'${rt.id}',versionId:'${ver.id}'})">戻る</button>
      <h1>${esc(stepLabel(step))}</h1>${routineMenuAction(rt.id)}</div>
  ...
  const rows = list.map((a) => `
    <div class="trick-row">
      <div class="head">
        ...
        <button class="btn small" onclick="sheetTrimAudio('${a.id}')">長さ</button>
      ...
      ${b.music ? (musicMissing && musicLoadedFor === "builder"
        ...
        : `<div class="music-name">♪ ${esc(b.music.name)}${musicMetaIsTrimmed(b.music) ? " ✂" : ""}</div>
           ...
           <div class="music-controls">
             ...
             <button class="music-pill" onclick="musicStop()">■ 停止</button>
             <button class="music-pill" onclick="sheetTrimBuilderMusic()">長さ</button>
           </div>`)
      : `<div class="row-2">
...
function renderHelp() {
  return `
    ...
    <div class="card">
      <h2>楽曲と録音</h2>
      <div class="help-body">編集画面で音源(MP3等)を添付すると、通し練習中に再生できます。<b>失敗をタップした瞬間の曲位置(♪1:23)が自動で記録</b>され、曲は自動停止。クリーン/中止を記録すると曲は頭に戻るので、次の通しは「▶ 再生」を押すだけです。<br><br>音源は「音源ライブラリ」に貯めて、どのルーティンでも「♪ ライブラリから」で使い回せます。マイク録音した音源もライブラリに追加できます。<br><br>音源はこの端末のブラウザ内にのみ保存され、JSONバックアップには含まれません。</div>
    </div>
    <div class="card">
      <h2>パート練習</h2>
...
function renderSettings() {
  ...
  return `
    ...
    <div class="card">
      <h2>編集画面で使う機能${infoBtn("editorFeatures")}</h2>
...
window.toggleSetting = (key, val) => {
  ...
  saveState();
  toast(val ? "表示する設定にしました" : "非表示にしました");
};
