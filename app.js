...
const APP_VERSION = "v154"; // 要望フォーム等で自動送信するアプリ版
const RUN_VIDEO_LIMIT = 5; // アプリ全体。6本目は自動削除せず、保存時に入れ替える
const RUN_VIDEO_BPS = 1500000; // 通し映像は振り返りやすさと容量のバランスを取り、約720pで記録
// 開発中は、演技全体を収めやすい3:4と、画面いっぱいに見せる9:16を撮影前に比較できるようにする。
// 3:4はブラウザによる中央クロップを避け、9:16は縦長出力を優先する。
const RUN_CAMERA_PROFILES = {
  wide: { id: "wide", label: "3:4 広め", width: 720, height: 960, ratio: 3 / 4, cssRatio: "3 / 4", resizeMode: "none" },
...
// 再生速度を変えても音程を維持する。旧Safari/Firefoxの互換プロパティも再設定する。
...
function runVideoAspect(video) {
  ...
  return video?.cameraProfile === "wide" ? RUN_CAMERA_PROFILES.wide.ratio : RUN_CAMERA_PROFILES.vertical.ratio;
}
...
function runCameraConfirmBody(routineId, status = "") {
  ...
  return `
    <div class="run-camera-head">
      <div><b>インカメで撮影</b><span>音声なし・音源停止まで</span></div>
    ...
    <small>選んだ画角で上の確認映像・撮影中プレビュー・保存映像を表示します。録画は楽曲の再生開始に合わせて始まり、停止すると終了します。</small>`;
}
...
function startRunVideoCapture(routineId) {
  ...
  try {
    const rec = mime
      ? new MediaRecorder(cap.stream, { mimeType: mime, videoBitsPerSecond: RUN_VIDEO_BPS })
      : new MediaRecorder(cap.stream, { videoBitsPerSecond: RUN_VIDEO_BPS });
    cap.rec = rec;
    cap.chunks = [];
    cap.recording = true;
...
function showRunVideoReview() {
  if (!pendingRunVideo) return;
  const profile = runVideoProfile(pendingRunVideo);
  showSheet(`
    <h3>通し練習の映像</h3>
    <div class="sheet-sub">${uiText(profile.label)} / ${uiText("音声なし")} / ${fmtTimeFine(pendingRunVideo.duration)} / ${fmtBytes(pendingRunVideo.size)}</div>
    <video class="run-video-review" style="${runVideoAspectStyle(pendingRunVideo)}" src="${pendingRunVideoUrl}" controls playsinline></video>
...
window.savePendingRunVideo = async (replaceId = "") => {
  ...
  const video = {
    ...
    camera: "user", audio: false, cameraProfile: pending.cameraProfile, aspectRatio: pending.aspectRatio,
  };
...
window.openRunVideo = async (id) => {
  ...
  const markers = found.run ? (found.run.events || []).filter((event) => event.videoTime != null).map((event) => {
    const rt = state.routines.find((r) => r.id === video.routineId);
    const ver = rt && found.sess ? getVersion(rt, found.sess.versionId) : null;
    const step = ver && ver.steps[event.stepIndex];
  ...
  showSheet(`
    ...
    <div class="sheet-sub">${uiText("インカメ")} / ${uiText(runVideoProfile(video).label)} / ${uiText("音声なし")} / ${fmtTimeFine(video.duration)}</div>
    <video id="run-video-player" class="run-video-review" style="${runVideoAspectStyle(video)}" src="${sheetVideoUrl}" controls playsinline></video>
        ...
        <footer class="home-libraries" aria-label="ライブラリ">
          <div class="home-library-grid">
            <button class="home-library-videos" onclick="go('runvideos')">
              <span class="home-library-copy"><b>演技映像ライブラリ</b><small>${runVideoCount ? `保存 ${runVideoCount}/${RUN_VIDEO_LIMIT}本` : "まだ映像はありません"}</small></span>
              <span class="home-button-arrow" aria-hidden="true">›</span>
            </button>
...
function renderRunVideos() {
  const list = [...storedRunVideos()].sort((a, b) => b.at - a.at);
  ...
  const rows = list.map((video, index) => {
    ...
    return `<article class="run-video-library-row">
      ...
      <div class="run-video-library-copy">
        ...
        <small>${fmtTimeFine(video.duration)} / ${fmtBytes(video.size || 0)} / ${english ? "Front camera · no audio" : "インカメ・音声なし"}</small>
      </div>
  ...
  return `
    <div class="topbar"><button class="back-btn" onclick="go('home')">戻る</button><h1>${english ? "Performance Video Library" : "演技映像ライブラリ"}</h1></div>
    <section class="card run-video-library-card" aria-labelledby="run-video-library-title">
      <div class="run-video-library-heading">
        <div><h2 id="run-video-library-title">${english ? "Saved full-run videos" : "保存した通し練習の映像"}</h2>
          <p>${english ? "Review, export, or delete recordings across all routines." : "すべてのルーティンの映像を、再生確認・書き出し・削除できます。"}</p></div>
        <strong>${list.length}/${RUN_VIDEO_LIMIT}</strong>
      </div>
      <div class="run-video-library-slots" aria-label="${english ? `${list.length} of ${RUN_VIDEO_LIMIT} video slots used` : `${RUN_VIDEO_LIMIT}本中${list.length}本を保存`}">
        ${Array.from({ length: RUN_VIDEO_LIMIT }, (_, index) => `<i class="${index < list.length ? "used" : ""}"></i>`).join("")}
      </div>
      ${totalBytes ? `<div class="run-video-library-size">${english ? "Storage used" : "使用容量"} ${fmtBytes(totalBytes)}</div>` : ""}
      <div class="run-video-library-list">${rows || `<div class="empty">${english ? "No performance videos yet.<br>Prepare the front camera before starting a full run." : "演技映像はまだありません。<br>通し練習の開始前にインカメを準備すると、終了後に保存できます。"}</div>`}</div>
    </section>`;
}
...
async function performRoutineDelete(id) {
  ...
  for (const video of storedRunVideos().filter((item) => item.routineId === id)) blobDel(video.blobId);
  ...
  return `
    ...
    ${runCamera && runCamera.recording ? `<div class="run-video-live" role="status" aria-live="polite">
      ...
      <div class="run-video-live-copy">
        ...
        <small>${runCameraProfile(runCamera.profileId).label}・インカメ・音声なし</small>
      </div>
    </div>` : ""}
    ${stoppedRunVideoCapture && stoppedRunVideoCapture.routineId === rt.id ? `<div class="run-video-stopped" role="status">
      <span>■</span><div><b>撮影終了</b><small>音源の停止に合わせて終了しました。通し結果を記録すると保存確認へ進みます。</small></div>
    </div>` : ""}
    ${musicCard}
    ${isOpen ? `<div class="openrun-note">この通しは記録して続行中 → 最後までいったら「完走」、失敗または実施できなかった技があれば該当箇所をタップ</div>` : ""}
    <div class="card">
...
window.undo = () => {
  ...
  if (sess && sess.runs.length) {
    ...
    if (video) {
      state.runVideos = storedRunVideos().filter((item) => item.id !== video.id);
      blobDel(video.blobId);
    }
    saveState(); render();
    ...
    <div class="card"><h2>通し練習の映像 <span class="card-count">全体 ${storedRunVideos().length}/${RUN_VIDEO_LIMIT}本</span></h2>
      ${runVideos.map((video) => `
        <div class="run-video-row">
          <div><b>${esc(runVideoTitle(video))}</b><span>${fmtTimeFine(video.duration)} / インカメ・音声なし</span></div>
...
function renderHelpEnglish() {
  return `
    ...
    <div class="card"><h2>Full-run workflow</h2>
      ...
      2. Tap the large vermilion start button, confirm the countdown, and begin. You can change the countdown with − / + on the confirmation screen. To film the run, choose 3:4 Wide or 9:16 Tall and prepare the front camera there. The setup preview uses the selected framing. Recording is video-only, starts with music playback, and stops when the music stops.<br>
      3. For a clean run, tap <b>Clean</b> below the skill list.<br>
      4. If something happens, tap the relevant skill. Choose an issue type, or choose <b>Not attempted</b> when an earlier issue prevented the next skill. Add optional cause tags or a note.<br>
  ...
  return `
    ...
    <div class="card">
      ...
      <div class="help-body">
        ...
        2. 朱色の「通し練習をスタート」→確認→カウントダウン。スタートから実施中まで「本日何本目」かを大きく表示し、スタートボタンにはこれまでの合計本数も表示します。初期値は5秒で、確認画面の−/＋からルーティンごとに変更できます。映像を残す場合は、同じ確認画面で3:4（広め）または9:16（縦長）を選び、「インカメを準備」を押します。最初の確認映像も選んだ画角で表示されます<br>
        3. 通しがノーミスなら、技一覧の一番下にある「クリーン」を1タップ<br>
        4. 失敗したら、落ちた技をタップ → 種類を選ぶ(初期値: ドロップして中止)。直前の失敗などで次の技ができなかった場合は、その技をタップして「実施できなかった」を選びます。中止なら前の技まで成功・後ろは未到達として自動処理。復帰して続けたら最後に「完走」<br>
        5. 次の1本はもう一度スタート。練習を終えたら「セッション終了」で振り返りと「次回試すこと」をメモします
      </div>
    </div>
    <div class="card">
      <h2>楽曲と録音</h2>
      <div class="help-body">編集画面で音源(MP3等)を添付すると、通し練習中に再生できます。編集画面・通し練習・パート練習の音量は共通で、各画面の音量バーから調整できます。<b>失敗をタップした瞬間の曲位置(♪1:23)が自動で記録</b>され、曲は自動停止。クリーン/中止を記録すると曲は頭に戻るので、次の通しは「▶ 再生」を押すだけです。<br><br>通し練習は開始確認から<b>インカメ・音声なし</b>で撮影できます。開発中は3:4（広め）と9:16（縦長）を選択でき、確認映像・撮影中プレビュー・保存後の再生枠へ同じ画角を反映します。録画は楽曲が実際に再生された時点で始まり、楽曲の一時停止・停止・終了に合わせて終了します。映像はアプリ全体で最大5本。6本目は古い映像を自動削除せず、保存時に入れ替える映像を選びます。分析画面から映像を開くと、失敗記録の3秒前へ移動できます。<br><br>音源は「音源ライブラリ」に貯めて、どのルーティンでも「♪ ライブラリから」で使い回せます。マイク録音した音源もライブラリに追加できます。音源と通し映像はこの端末のブラウザ内にのみ保存され、JSONバックアップには含まれません。</div>
    </div>
    <div class="card">
      <h2>パート練習</h2>
    ...
    <div class="card">
      <h2>データの保存</h2>
      <div class="help-body">データはこの端末のブラウザ内に保存されます。iPhoneは長期間使わないと保存データを消すことがあるため、<b>定期的に設定からJSONバックアップを書き出してください</b>。機種変更時もJSONで移行できます。音声と映像はJSONに含まれないため、残したい通し映像は分析画面から書き出してください。</div>
    </div>`;
}
