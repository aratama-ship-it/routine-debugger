...
const APP_VERSION = "v51"; // 要望フォーム等で自動送信するアプリ版
...
function renderEdit() {
  ...
  if (hasEditorMusic && draft.steps.length) {
    ...
    timelineCard = `
    <div class="card">
      <h2>タイムライン (各技の長さで曲位置を計算 — 合計 ${fmtTime(total)}${songDur ? ` / 曲 ${fmtTime(songDur)}` : ""})${over ? ` <span style="color:var(--danger)">曲より長い</span>` : ""}</h2>
      ...
      <p class="hint">各技の長さ(動画リンクは自動、移行は±で調整)を足して曲位置を計算します。「自動セット」で全ステップの♪キューに反映。上のプレイヤーで再生するとバーに現在位置が出ます。</p>
    </div>`;
  }
  return `
...
function renderAudios() {
  ...
  return `
    ...
    <p class="hint">これらは、ルーティン編集/タイムラインの「♪ ライブラリから」で選んで使えます。付属サンプルは最初から使えます。音源はこの端末内に保存され、JSONバックアップには含まれません(残したい録音は書き出しを)。</p>`;
}
