/* ルーティンノート — 通し中の「いまこの技」表示
 *
 * 技と技のあいだの空白時間(空間)に、直前の技の名前と動画が出たままになっていた。
 * 何もしない時間なのに技が流れていると、いま何をすべきかを取り違える。
 * 空間は空間として見せる。
 *
...
(() => {
  ...
  // 直前にキューを過ぎたステップを採用するが、その技がすでに終わっていて次がまだ
  // 始まっていなければ gap=true とし、表示側で「空間」を出す。
  // 最後の技より後ろは空間ではなく「フィニッシュ」として扱う(次が無いので gap にしない)。
